			     ...
			     struct Packet *pktin)
{
  ...
  begin_key_exchange:
    ...
    if (!s->hkey && ssh->kex->main_type != KEXTYPE_GSS) {
	...
	if (!gss_authenticated)
        ...
        /*
         * We do not care if the server changes host keys after the
         * initial KEX.  However, if the initial key was strongly
         * authenticated with non-warning algorithms (not definitively
         * knowable with GSSKEYEX), then we should store the new key.
         */
        s->keystr = ssh->hostkey->fmtkey(s->hkey);
        if (!ssh->warned && !ssh->initial_used_gss &&
            strcmp(ssh->hostkey_str, s->keystr)) {
            store_host_key(ssh->savedhost, ssh->savedport,
                           ssh->hostkey->keytype, s->keystr);
            sfree(ssh->hostkey_str);
            ssh->hostkey_str = s->keystr;
